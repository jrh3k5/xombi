import { XMTPClientFactory, XMTPConfig, XMTPInstallationLimitError } from './xmtp_client_factory';
...
describe('XMTPClientFactory', () => {
  ...
  describe('validateConfig', () => {
    it('should throw error for missing signer key', () => {
      ...
      expect(() => XMTPClientFactory.validateConfig(config))
        .toThrow('invalid Xombi signer key; must be of type `0x${string}`');
    });
    ...
    it('should throw error for missing encryption key', () => {
      ...
      expect(() => XMTPClientFactory.validateConfig(config))
        .toThrow('invalid XMTP encryption key; must be of type `0x${string}`');
    });
    ...
    it('should throw error for invalid environment', () => {
      ...
      expect(() => XMTPClientFactory.validateConfig(config))
        .toThrow('invalid XMTP_ENV: invalid');
    });
  ...
  describe('parseEnvironmentConfig', () => {
    ...
    it('should throw error for missing required environment variables', () => {
      expect(() => XMTPClientFactory.parseEnvironmentConfig())
        .toThrow('invalid Xombi signer key; must be of type `0x${string}`');
    });
  });
  ...
  describe('createClient', () => {
    ...
    it('should create client successfully', async () => {
      ...
      jest.requireMock('viem').toBytes.mockReturnValue(new Uint8Array([1, 2, 3, 4]));
      jest.requireMock('viem/accounts').privateKeyToAccount.mockReturnValue(mockAccount);
      jest.requireMock('./eoa').convertEOAToSigner.mockResolvedValue(mockSigner);
      jest.requireMock('@xmtp/node-sdk').Client.create.mockResolvedValue(mockClient);
    ...
    it('should throw XMTPInstallationLimitError when installation limit reached without auto-revoke', async () => {
      const installationError = new Error('Cannot register a new installation because the InboxID abc123 has already registered 20/10 installations');
      
      jest.requireMock('viem').toBytes.mockReturnValue(new Uint8Array([1, 2, 3, 4]));
      jest.requireMock('viem/accounts').privateKeyToAccount.mockReturnValue({ address: '0xmockaddress' });
      jest.requireMock('./eoa').convertEOAToSigner.mockResolvedValue({ sign: jest.fn() });
      jest.requireMock('@xmtp/node-sdk').Client.create.mockRejectedValue(installationError);

      await expect(XMTPClientFactory.createClient(mockConfig))
        .rejects.toThrow(XMTPInstallationLimitError);
    });

    it('should auto-revoke and retry when installation limit reached with auto-revoke enabled', async () => {
      const configWithAutoRevoke = { ...mockConfig, autoRevokeInstallations: true };
      const installationError = new Error('Cannot register a new installation because the InboxID abc123 has already registered 20/10 installations');
      const mockClient = { id: 'mock-client' };
      const mockAccount = { address: '0xmockaddress' };
      const mockSigner = { sign: jest.fn() };
      ...
      jest.requireMock('@xmtp/node-sdk').Client.create
        .mockRejectedValueOnce(installationError)
        .mockResolvedValueOnce(mockClient);

      // Mock inbox state and revocation
      jest.requireMock('@xmtp/node-sdk').Client.inboxStateFromInboxIds.mockResolvedValue([{
        installations: [
          { bytes: new Uint8Array([1]) },
          { bytes: new Uint8Array([2]) },
        ]
      }]);
      jest.requireMock('@xmtp/node-sdk').Client.revokeInstallations.mockResolvedValue(undefined);

      const result = await XMTPClientFactory.createClient(configWithAutoRevoke);

      expect(result.client).toBe(mockClient);
      expect(jest.requireMock('@xmtp/node-sdk').Client.revokeInstallations).toHaveBeenCalledWith(
        mockSigner,
        'abc123',
