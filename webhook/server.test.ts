...
describe('WebhookServer', () => {
  ...
  beforeEach(() => {
    ...
    server = new WebhookServer(mockRequestTracker, mockOmbiToken, mockAllowlistedIPs, true);
  ...
  describe('middleware and security', () => {
    ...
    it('should reject requests from non-allowlisted IP', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith('Rejected unauthorized webhook request from:', expect.any(String));
    });
  ...
  describe('webhook payload processing', () => {
    ...
    it('should process availability notification with movie', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith('Sent notification to 0x1234567890abcdef for The Matrix');
    ...
    it('should ignore notifications for unknown request ID', async () => {
      ...
      expect(mockNotificationHandler).not.toHaveBeenCalled();
      expect(console.log).toHaveBeenCalledWith('No requester found for request ID: 999');
    });
    ...
    test.each([
      ...
      ])('should detect availability keywords in event type %s', async (eventType: string, shouldNotify: boolean) => {
        const payload: WebhookPayload = {
          eventType: eventType,
    ...
    test.each([
        ...
        async (subject: string | undefined, message: string | undefined, shouldNotify: boolean) => {
        const payload: WebhookPayload = {
          eventType: 'TestEvent',
          subject: subject,
          message: message,
  ...
  describe('server lifecycle', () => {
    it('should start server on specified port', async () => {
      const testServer = new WebhookServer(mockRequestTracker, mockOmbiToken, mockAllowlistedIPs);
      
      await testServer.start(0); // Use port 0 to get available port
      expect(console.log).toHaveBeenCalledWith(expect.stringMatching(/Webhook server running on port \d+/));
  ...
  describe('setNotificationHandler', () => {
    it('should work without notification handler set', async () => {
      const serverWithoutHandler = new WebhookServer(mockRequestTracker, mockOmbiToken, mockAllowlistedIPs);
      mockRequestTracker.setRequester('123', '0x1234567890abcdef');
